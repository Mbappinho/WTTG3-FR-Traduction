...
- **Option AZERTY (ZQSD)** : pak `WTTGSD-Windows_FR_AZERTY_P` (`build_azerty_imc_patch.py`) — NameMap IMC `W→Z`, `A↔Q`. **Validé in-game** (2026-07-28). Mini-jeux hack restent en W/A (natif) : doc joueur = touches QWERTY sur hacks **ou** pas de mod AZERTY. Injecteur v1.6.0 **retiré** en v1.6.1 (fatal errors).
...
- **v1.6.0** : tentative AZERTY + injecteur UE4SS/AHK (mini-jeux) — **retire en 1.6.1** (fatal errors).
- **v1.6.1** : retrait injecteur ; AZERTY = pak ZQSD seul ; INSTALLER nettoie dwmapi/AHK ; doc mini-jeux W/A vs pas de mod.
