...
- **Option AZERTY (ZQSD)** : pak séparé `WTTGSD-Windows_FR_AZERTY_P` via `scripts/build_azerty_imc_patch.py` — NameMap IMC `W→Z`, `A↔Q`. Vanilla = W ; avec le pak + Windows AZERTY = Z avance. Full = prompt O/N ; Nexus = `optionnel_azerty\`. Saisie texte inchangée. **Validé in-game** (2026-07-28). Depuis **v1.6.0** Full : + runtime UE4SS/AHK pour mini-jeux (MemDealloc/ShiftSEQ) ; INSTALLER relance apres maj GitHub.
...
- **v1.6.0** : AZERTY **complet** (Full) — pak IMC + runtime UE4SS/AutoHotkey pour mini-jeux hack ; apres maj GitHub, INSTALLER **relance** le nouveau script (un clic). Nexus : pak seul. Attribution : logique mini-jeux inspiree du mod communautaire AZERTY Rebind.
